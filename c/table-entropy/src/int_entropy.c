...
void main()
{
	...
	for(uint32_t rows = min_rows; rows <= max_rows; rows += row_iterator)
	{
		...
		{			
			// Create an uint32_t array that represents a rows x cols table
			uint32_t *table = malloc(rows * cols * sizeof(uint32_t));

			// (re)seed the random number generator
			srand(time(NULL));

			// create a rows by cols table of integers
			for(uint32_t row=0; row < rows; row++)
			{
				for (uint32_t col=0; col < cols; col++)
				{
					table[row * cols + col] = rand() % 32767;
				}
			}
